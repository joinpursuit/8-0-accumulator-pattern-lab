...
function isValid(rolls) {
  ...
  // if array is equal to all numbers return true
  // otherwise return false
  // Declare a flag variable and assign it the vaule of true
  let allNumbers = true;
  // loop through rolls array
  for (let i = 0; i < rolls.length; i++) {
    // check if all values in rolls array are numbers
    // Declare a variable and assign it to the current index
    let roll = rolls[i] 
...
function filterOutLowValues(rolls, lowest) {
  ...
  for (let i = 0; i < rolls.length; i++) {
  let num = rolls[i];
    // Iniside of the loop check every array for numbers greater than or equal the 'lowest' value
  if (num >= lowest) {
